...
namespace bookcity
{
    public partial class frm_code : Form
    {
        DataSet v_ds = new DataSet("d_code");
        str_code v_str_code = new str_code();
        public frm_code()
        {
            InitializeComponent();
        }
        ...
        private void p_select_code()
        {
            string v_ret;
            string v_xml;
            v_xml = cls_book.f_select_code(out v_ret);
        ...
        private void cmd_insert_code_Click(object sender, EventArgs e)
        {
            ...
            if (f_val())
            {
                ...
                cls_book.f_insert_code(v_str_code, out v_ret);
                MessageBox.Show(v_ret);
                p_select_code();
            }
        ...
        private void cmd_update_code_Click(object sender, EventArgs e)
        {
            ...
            if (f_val())
            {
                ...
                v_str_code.def_yn = chk_def_yn.Checked.ToString();
                v_str_code.sort_order = Convert.ToInt32(txt_sort_order.Text);
                cls_book.f_update_code(v_str_code, out v_ret);
                MessageBox.Show(v_ret);
                p_select_code();
        ...
        private void cmd_delete_code_Click(object sender, EventArgs e)
        {
            ...
            v_str_code.datanum = Convert.ToInt32(txt_datanum.Text);
            cls_book.f_delete_code(v_str_code, out v_ret);
            MessageBox.Show(v_ret);
            p_select_code();
        }
    }
